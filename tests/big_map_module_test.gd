...
func _test_canvas_contract() -> void:
	...
	var pin_size: Vector2 = canvas_constants.get("TRACKING_PIN_DRAW_SIZE", Vector2.ZERO) as Vector2
	var normal_pin_geometry: Dictionary = canvas.call("_tracking_pin_visual_geometry", Vector2(120.0, 140.0), false)
	var selected_pin_geometry: Dictionary = canvas.call("_tracking_pin_visual_geometry", Vector2(120.0, 140.0), true)
	_check(pin_size.x >= 30.0 and pin_size.y >= 38.0, "Agulha normal continua pequena para uso operacional.")
	_check(normal_pin_geometry.get("size", Vector2.ZERO) == selected_pin_geometry.get("size", Vector2.ONE), "Selecao da agulha alterou sua escala.")
	_check(normal_pin_geometry.get("rect", Rect2()) == selected_pin_geometry.get("rect", Rect2(1, 1, 1, 1)), "Selecao da agulha alterou seu retangulo.")
