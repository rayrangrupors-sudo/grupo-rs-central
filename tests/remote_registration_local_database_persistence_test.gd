...
class QueueHost:
	...
	func _ensure_local_database_modification_saved(_serial: String, _expected_product: Dictionary) -> Dictionary:
		...
		if scenario == "local_database_error":
			return {"ok": false, "message": "Banco local SQL indisponivel para confirmacao."}
		return {"ok": true}
...
func _run() -> void:
	for queue_path in [
		...
		await _check_local_database_failure(queue_path)
	...
	if failed:
		...
		print("REMOTE_REGISTRATION_BANCO_LOCAL_SQL_PERSISTENCE_TEST_OK")
		quit(0)
...
func _check_successful_new_registration(queue_path: String) -> void:
	...
	_expect(int(host.local_database_calls) == 1, "%s: cadastro novo nao confirmou a persistencia no Banco local SQL." % queue_path)
...
func _check_api_failure(queue_path: String) -> void:
	...
	_expect(int(host.local_database_calls) == 0, "%s: falha da API tentou gravar no Banco local SQL." % queue_path)
	queue.queue_free()
	host.queue_free()


func _check_local_database_failure(queue_path: String) -> void:
	var host := QueueHost.new()
	host.scenario = "local_database_error"
	root.add_child(host)
	var queue := _new_queue(host, queue_path)
	var queue_id := _enqueue(host, queue, "100000003")
	var item := await _wait_for_queue(queue, queue_id)
	_expect(str(item.get("state", "")) == "pending", "%s: falha do Banco local SQL nao preservou a operacao como pendente." % queue_path)
	_expect(int(host.finalize_calls) == 1, "%s: falha do Banco local SQL descartou a atualizacao local." % queue_path)
	_expect(int(host.local_database_calls) == 1, "%s: falha do Banco local SQL nao foi confirmada uma vez." % queue_path)
	_expect(host.records.size() == 1, "%s: falha do Banco local SQL duplicou o registro local." % queue_path)
...
func _check_confirmation_pending(queue_path: String) -> void:
	...
	_expect(int(host.local_database_calls) == 0, "%s: confirmacao remota pendente tentou Banco local SQL antes da leitura remota." % queue_path)
...
func _check_reopen_sync_and_table_after_confirmation(queue_path: String) -> void:
	...
	_expect(host.table_rows.size() == 1, "%s: tabela nao foi atualizada apos API, Store e Banco local SQL confirmados." % queue_path)
