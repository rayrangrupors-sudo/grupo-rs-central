...
func _location_monitoring_status(location: Dictionary) -> Dictionary:
	if bool(location.get("position_preserved", false)):
		return _set_location_communication_state(location, "Desatualizado", YELLOW)
	if not _location_coordinates_valid(location):
		return _set_location_communication_state(location, "Sem posição", MUTED)
	var updated_at := str(location.get("updated_at", "")).strip_edges()
	if updated_at == "":
		return _set_location_communication_state(location, "Sem comunicação", MUTED)
	var hours := _hours_since_grupo_rs_datetime(updated_at)
	if hours < 0.0:
		return _set_location_communication_state(location, "Sem comunicação", MUTED)
	var ignition_state := _location_ignition_state(location.get("ignition", null))
	# ST310 ligado transmite a cada minuto; desligado, a cada hora. O limite
	# inclui margem para atraso da API sem esconder uma posição realmente antiga.
	var stale_limit_hours := (5.0 / 60.0) if ignition_state == 1 else 2.0
	if hours >= stale_limit_hours:
		return _set_location_communication_state(location, "Desatualizado", YELLOW)
	if ignition_state == 1:
		return _set_location_communication_state(location, "Ligado", GREEN)
	if ignition_state == 0:
		return _set_location_communication_state(location, "Desligado", RED)
	var fallback := _location_device_status(location)
	var fallback_color: Color = fallback.get("color", MUTED)
	return _set_location_communication_state(location, str(fallback.get("label", "Sem status")), fallback_color)


func _set_location_communication_state(location: Dictionary, label: String, color: Color) -> Dictionary:
	# O mapa recebe o mesmo estado usado pela linha e pelo painel de detalhes;
	# isso evita que o ponto fique verde enquanto o texto indica outra situacao.
